...
export type CategoryEntry = {
    text: string,
    category: Record<string, (ExampleDataEntry | string)>
};
export type ExampleDataType = Record<string, CategoryEntry>;
...
const normalizedExamplesData = Object.entries(examplesData as ExampleDataType)
    .reduce((directoryAcc, [categoryKey, categoryExamples]) =>
        ({
            ...directoryAcc,
            [categoryKey]: Object.keys(categoryExamples.category).reduce((categoryAcc, categoryExampleOrString) =>
            {
...
{
    return Object.entries(examplesData as ExampleDataType).map(([folderKey, folderEntries]) =>
    {
        const options = Object.keys(folderEntries.category).reduce((acc, exampleDataEntry) =>
        {
